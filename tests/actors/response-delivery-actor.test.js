...
test('OUTCOMES constants are the locked ten-way set (GM-22 through GM-29)', () => {
  // GM-22 introduced executed / abstained / rejected. GM-23 added
  // `staged`. GM-24 added `recorded`. GM-25 added
  // `authorized_recorded`. GM-26 added `claim_recorded`. GM-27
  // added `attempt_recorded`. GM-28 added `outcome_recorded`.
  // GM-29 added `verification_recorded`. The shared OUTCOMES
  // enum is the locked vocabulary; any addition fails this test
  // and forces a paired review of the actor boundary doc and
  // the adversarial snapshot (C4).
  assert.deepEqual(
    Object.values(OUTCOMES).sort(),
    [
      'abstained', 'attempt_recorded', 'authorized_recorded',
      'claim_recorded', 'executed', 'outcome_recorded', 'recorded',
      'rejected', 'staged', 'verification_recorded',
    ]
  );
});
