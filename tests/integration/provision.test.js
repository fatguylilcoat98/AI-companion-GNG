 ...
 * --force refusal paths.
 *
 * Requires DATABASE_URL (set by the integration-tests CI job to a
...
before(async () => {
  assert.ok(DATABASE_URL, 'DATABASE_URL must be set for the provisioning integration tests');
...
test('provision: happy path seeds the four rows and the runtime reaches ready', async () => {
  ...
  const result = runScript(['--answers', answersFile], { DATABASE_URL });
  ...
  assert.ok(events.some((e) => e.event === 'setup.companion_profile.created'), 'companion_profile.created must be emitted');

  // The runtime now reaches ready against the seeded DB.
  const handle = await boot(
    { DATABASE_URL, PORT: '13578', LYLO_SHELL_MODE: 'true' },
...
test('provision: --force does not perform destructive reseed in this version', async () => {
  ...
  const answersFile = writeAnswersFile(buildAnswers());
  const result = runScript(['--answers', answersFile, '--force'], { DATABASE_URL });
  fs.unlinkSync(answersFile);
...
test('provision: missing answers file path is rejected', async () => {
  const result = runScript([], { DATABASE_URL, ANSWERS_FILE: '' });
  assert.equal(result.status, 1);
  const events = parseLogLines(result.stdout);
  assert.ok(
