...
| Actors boundary guard | `check-actors-boundary.js` | In `src/actors/` (GM-22 through GM-29): zero SQL keywords; the identifier `insertPrivateMemory` is forbidden; `pg`, every model SDK (including `@anthropic-ai/sdk` — the conversation runtime owns that boundary), HTTP/server framework, `child_process`, `worker_threads`, `cluster`, and scheduling (`setInterval`/`setImmediate`/`cron`/`schedule` — `setTimeout` permitted because the conversation runtime uses it transitively) imports / identifiers are forbidden; `fs` write API surface is forbidden; streaming + tool-calling identifiers are forbidden; cross-layer imports of `../runtime`/`../db`/`../setup`/`../memory`/`../companion` are rejected; imports of `../governance`, `../conversation`, and (GM-23) `../review` are restricted to the public entries. GM-24's review-decision actor, GM-25's execution-authorization actor, GM-26's execution-claim-ledger actor, GM-27's execution-attempt-ledger actor, GM-28's execution-outcome-ledger actor, and GM-29's execution-verification-ledger actor all reuse the existing `../review` public-entry allowance. See `actor-runtime-boundary.md`. |
| Review boundary guard | `check-review-boundary.js` | In `src/review/` (GM-23 through GM-29): SQL keywords `UPDATE`/`DELETE`/`DROP`/`ALTER`/`TRUNCATE`/`GRANT`/`REVOKE`/`CREATE` are forbidden (`INSERT` is permitted but tracked separately); every `FROM`/`JOIN` references the review-module read allowlist (the seven governance-staging tables + `users` + `pilot_instances`); every `INSERT INTO` references the tighter write allowlist (the seven governance-staging tables); the identifier `insertPrivateMemory` is forbidden; `pg` is imported only by `src/review/client.js`; every model SDK, HTTP/server framework, `child_process`/`worker_threads`/`cluster`, scheduling identifiers, `fs` write API surface, streaming + tool-calling identifiers are forbidden. **File-scoped extensions** (GM-26 OQ-26.14 + GM-27 OQ-27.14 + GM-28 OQ-28.14 + GM-29 OQ-29.10(b), refactored into shared `runFileScopedForbiddenScan` helper per OQ-27.17): `src/actors/execution-claim-ledger-actor.js` must not contain `executed`/`completed`/`dispatched`/`delivered`/`finalized`/`succeeded`/`failed` as bare identifiers; `src/actors/execution-attempt-ledger-actor.js` must not contain the same seven words **plus `committed`** (one word stricter — DB commit lives in the transaction layer, not the actor file); `src/actors/execution-outcome-ledger-actor.js` must not contain the same eight words **plus ten truth-claim words** (`verified`/`confirmed`/`actual`/`actually`/`definitely`/`proven`/`certain`/`real`/`reality`/`truth`) — 18 words (AN OUTCOME ROW IS NOT TRUTH); `src/actors/execution-verification-ledger-actor.js` must not contain a different shape of 20 words — 12 operational/repair (`executed`/`dispatched`/`retry`/`retried`/`reconcile`/`reconciled`/`rollback`/`compensate`/`side_effect`/`mutate`/`promote`/`admit`) + 8 fix-it temptation (`fix`/`repair`/`correct`/`heal`/`resolve`/`revert`/`undo`/`apply`) — bare `execute` and `dispatch` deliberately omitted to avoid colliding with the actor contract method name (VERIFICATION ≠ RECONCILIATION ≠ REPAIR). See the seven governance-staging boundary docs. |
| RLS / privacy contract | `tests/rls-contract/run-contract.js` + `tests/rls-contract/run-real.test.js` | RLS / privacy contract, two suites run serially in the same CI job: (1) **synthetic** — applies a generic schema, the candidate policies, and two-pilot fixtures to a throwaway Postgres; (2) **real-schema** (GM-15) — applies `db/migrations/*.sql` (now through `014_execution_verifications.sql`) and the same fixtures, then runs the matrix against the real schema. Both suites assert the visibility / write matrix (cross-pilot isolation, memory-store rules per visibility level, vault-session row-state model, admin denial on private memories, default-deny, the GM-23 review-queue policies, the GM-24 review-decisions policies + self-review trigger + UNIQUE, the GM-25 execution-authorizations admin-only INSERT/SELECT + preconditions trigger + UNIQUE, the GM-26 execution-claims admin-only INSERT/SELECT + BEFORE-INSERT preconditions trigger + UNIQUE, the GM-27 execution-attempts admin-only INSERT WITH CHECK + admin-only SELECT + 5-deep BEFORE-INSERT trigger + UNIQUE on execution_claim_id, the GM-28 execution-outcomes admin-only INSERT WITH CHECK + admin-only SELECT + 6-deep BEFORE-INSERT trigger + UNIQUE on execution_attempt_id + CHECK on outcome_type, and the GM-29 execution-verifications admin-only INSERT WITH CHECK + admin-only SELECT, the BEFORE-INSERT preconditions trigger (outcome-exists / self-verification / 7-deep chain walk), UNIQUE on execution_outcome_id, append-only trigger, CHECK locking verification_type to the 4-value channel vocabulary and verification_result to the 3-value vocabulary). See `rls-privacy-contract.md`. |
| Gauntlet boundary guard | `check-gauntlet-boundary.js` | In `src/gauntlet/` (GM-30): zero SQL keywords; the identifier `insertPrivateMemory` is forbidden; `pg`, every model SDK, HTTP/server framework, `child_process`/`worker_threads`/`cluster`, scheduling identifiers, `fs` write API surface are forbidden; relative imports restricted to the three public surfaces (`../governance`/`../actors`/`../review` top-level entries only — no deeper, no other `src/` layer); the 7-word L24 forbidden-vocabulary scan (`bypass`/`skip`/`disable`/`override`/`force`/`monkeypatch`/`monkey_patch`); reciprocity enforced by the six other boundary guards (runtime/memory/companion/conversation/actors/review) — each refuses any `../gauntlet` import (GM-30 OQ-30.12). The gauntlet is **test-only**; production runtime never imports it. See `gauntlet-harness.md`. |
| Gauntlet contract tests | `tests/gauntlet/runner.test.js` + `tests/gauntlet/scenarios/*.json` | The adversarial gauntlet suite (GM-30). Loads every versioned scenario JSON, runs each end-to-end through the harness against a real Postgres (drops + re-applies schema + fixtures per scenario per OQ-30.5), and asserts the actual result matches the declared `expect` block. **Versioned scenarios block CI** (per OQ-30.8(a)); the gauntlet runs in the existing `integration-tests` job and reuses the same `lylo_app_login` LOGIN role. Manual scenarios under `tests/gauntlet/manual/` are gitignored and never auto-run; the runner requires the explicit `--manual` flag (per OQ-30.15(a), L38-enforced). See `gauntlet-harness.md`. |

All fifteen guards (fourteen baseline + the new gauntlet
boundary) plus the RLS / privacy contract plus the gauntlet
contract tests are **enforced** — a violation fails the build.
