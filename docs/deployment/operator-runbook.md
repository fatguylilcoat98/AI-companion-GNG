...
#### Memory + companion + conversation + governance + actors + review are all library-only today (GM-17 through GM-26)
...
substrate, the GM-25 execution-authorization substrate, and the
GM-26 execution-claim substrate (extended `src/review/` + four
Decision-gated actors in `src/actors/`) are all **libraries**. No process in this
...
review decisions, recorded execution authorizations, OR recorded
execution claims** (approval is NOT authorization; authorization
is NOT execution; an authorization row is NOT an execution
signal; a claim row is NOT execution — it ONLY records single-
consumption), and any mounting of the actor / classifier /
conversation runtime / review substrate into a production
process all remain explicitly deferred behind their own decision
gates. The system is **internal-build only** until the council's
...
GM-23, GM-24, GM-25, and GM-26 reuse **this same** LOGIN role
for all four governance-staging substrates —
`src/review/client.js` connects via `LYLO_APP_DATABASE_URL` (no
new env var, no new LOGIN role). If you are deploying only the
runtime shell, you may leave `LYLO_APP_DATABASE_URL` unset and
skip the `lylo_app_login` LOGIN role — `npm start` will succeed
without them. (The CI integration-tests job sets them because
the GM-17/GM-18/GM-19/GM-20/GM-23/GM-24/GM-25/GM-26 integration suites exercise
all five libraries through the same LOGIN role a future
...
| `LYLO_APP_DATABASE_URL` | Memory-governance module (GM-17) + review-queue (GM-23) + review-decision (GM-24) + execution-authorization (GM-25) + execution-claim (GM-26) substrates | Connection string for a LOGIN role whose effective identity is `lylo_app` (SELECT on memory + supporting tables; INSERT on `memory_store`, `governance_audit_log`, and the four governance-staging tables `governance_review_queue`, `governance_review_decisions`, `governance_execution_authorizations`, `governance_execution_claims`; `UPDATE (revoked_at)` on `memory_vault_sessions`). **DO NOT grant BYPASSRLS** — see "LOGIN role provisioning" below. Read by `src/memory/client.js` and `src/review/client.js`. Required only when a process consumes the memory or review modules; `parseEnv` does not require it because boot does not mount either. |
