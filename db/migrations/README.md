...
Migrations `001`–`010` are in place: the GM-3 baseline schema,
the GM-15 RLS / privacy policies, the GM-23 review-queue
substrate, the GM-24 review-decision substrate, and the GM-25
execution-authorization substrate. The master starts a **clean**
migration chain — no historical or archived SQL is carried over
from any reference system.
...
- **Admissibility lifecycle.** GM-3 ships the `admissibility_state`
  ...
  remain deferred.
- **Execution consumer.** GM-25 records explicit human
  authorizations — but a recorded authorization is **not** an
  execution signal. Future execution capabilities would need a
  separately gated decision, a separately gated boundary guard,
  a separately gated adversarial review, and explicit semantics
  for revocation, expiry, and replay protection (none of which
  exist in GM-25).
- **Authorization revocation / expiry.** GM-25 ships the artifact
  only — authorizations are append-only and have no `revoked_at`
  or `valid_until`. Future GMs may introduce these in paired
  changes.
- **Consumer of `governance_execution_authorizations` /
  `governance_review_decisions`.** No production code reads
  either table for any operational purpose. The G13 adversarial
  static-scan canary asserts zero references outside the
  documented writing path.
